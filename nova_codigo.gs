...
function doPost(e) {
  try {
    ...
    if (action === 'send_corte_email') {
      ...
      if (body.qrPngBase64) {
        const qrBlob = Utilities.newBlob(Utilities.base64Decode(body.qrPngBase64), 'image/png', 'corte_qr.png');
        opciones.inlineImages = { corte_qr: qrBlob };
        // TEMPORAL, para diagnosticar por qué el QR inline no se ve en
        // algunos clientes: se manda TAMBIÉN como adjunto normal
        // descargable. Si el adjunto sí abre como un QR válido, el
        // problema es solo la referencia cid: inline, no la imagen en sí.
        opciones.attachments = [qrBlob];
      }
      MailApp.sendEmail(opciones);
      return json({ok:true, qrBytesLength: body.qrPngBase64 ? Utilities.base64Decode(body.qrPngBase64).length : 0});
    }
